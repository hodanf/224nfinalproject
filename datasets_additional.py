...
def load_multitask_data(sentiment_filename,paraphrase_filename,similarity_filename,similarity_filename2,split='train'):
    sentiment_data = []
    num_labels = {}
    if split == 'test':
        with open(sentiment_filename, 'r') as fp:
    ...
    if split == 'test':
        ...
        with open(similarity_filename2, 'r') as fp:
            for record in csv.DictReader(fp,delimiter = '\t'):

                sent_id = record['pair_ID'].lower().strip()
                similarity_data.append((preprocess_string(record['sentence_A']),
                                        preprocess_string(record['sentence_B']),
                                        float(record['relatedness_score']),sent_id))
...
def load_multitask_data2(sentiment_filename,paraphrase_filename,similarity_filename,split='train'):
    sentiment_data = []
    num_labels = {}
    if split == 'test':
        with open(sentiment_filename, 'r') as fp:
